...
#
# ./look_for_more_words-SCI.py filename
# 
# The program opens the file and filters the words aginst a set of dictionaries.
#
# Examples:
# ./look_for_more_words-SCI.py /tmp/unique_words-abstracts-English.json
# ./look_for_more_words-SCI.py /tmp/paired-keywords-SCI.json
#
...
import miss_spelled_to_correct_spelling_SCI
import common_acronyms_SCI
import diva_merged_words_SCI
...
words_to_ignore=[
    ...
    '&int;<span',
    '<sub',
    '0.1rem;\">a</sub></span></span>(f&Prime;(x))<sup>2</sup>',
    'dx',
    '&lambda;',
    '&sum;<span',
    '0.05rem;\">i=1</sub></span></span>(f(x<sub>i</sub>)',
    'y<sub>i</sub>)<sup>2</sup></em>',
    '[]',
    '&alpha;<sub>s</sub>',
    '&theta;=',
    '0<sub>0</sub>',
    ...
    'f\\cdot\\nabla_xu',
    'l/100',
    '&OpenCurlyDoubleQuote;Game',
    'Generals&CloseCurlyDoubleQuote',
    '<em>f</em>&middot;&nabla;<sub',
    'style=\"position',
    '&Delta;(i&omega',
    '(540)</em>~km',
    '$T_Q/T_{tot',
    ...
    '‑1',
    '0&lt;i&lt;j&lt;n+1',
    'alue', # the "v" has beeb separated from this string
    '\\mathcal{L',
    '\\mathbb{F}_4',
    '\\mathbb{F}_q',
    '\\in',
    ...
    '<em>0.3',
    '&leq;',
    '<em>S<sup>1</sup>&times',
    'S<sup>1</sup></em>',
    '<em>T(x,y)',
    'x+&omega;,x+f(y))</em>',
    '<em>T<sub>Q</sub>/T<sub>tot</sub>',
    '<em>R<sub>P',
    'max</sub></em>',
    '<em>R<sub>D',
    '<strong>optimal',
    ...
    'flex-direction',
    '<em>f</em>',
    '&part;<sub><em>t</em></sub>',
    '<em>u</em>',
    '<em>f</em>&middot;&nabla;<sub><em>x</em></sub><em>u</em>',
    'cite{Zparq',
    '\\cite{Park4',
    '\\cite{Roper2010',
    '\\cite{Zparq',
    '\\cite{complex',
    '\\cite{flagser',
    '<em>E</em>[sup<sub><em>t&isin;T</em></sub>',
    '<em>X</sub>t</sub></em>]',
    'Z&rightarrow;b<span',
    'overline;\">b</span>',
    'ZH&rightarrow;b<span',
    'H&rightarrow;b<span',
    '<em>RNG',
    'k−&epsilon;</em>',
    'SST</em>',
    ...
    "overline;\">b</span>&gamma;&gamma",
    "HH&rightarrow;b<span",
...
def main():
    ...
    for e in common_acronyms_SCI.well_known_acronyms_list:
        if len(e) >= 1:
            ack=e[0]
            if len(e) >= 2:
                d=e[1]
    ...
    print(f'{(len(well_known_acronyms)):>{Numeric_field_width}} unique acronyms in ({len(common_acronyms_SCI.well_known_acronyms_list)}) well_known_acronyms_SCI')
    ...
    for w in words_KTH_swedish:
        ...
        if w not in common_swedish.KTH_ordbok_Swedish_with_CEFR:
            ...
            if w in well_known_acronyms:
                continue
            if w in miss_spelled_to_correct_spelling_SCI.miss_spelled_to_correct_spelling:
                continue
    ...
    print(f'{len(diva_merged_words_SCI.merged_words):>{Numeric_field_width}} words in merged_words SCI')
    print(f'{len(miss_spelled_to_correct_spelling_SCI.miss_spelled_to_correct_spelling):>{Numeric_field_width}} words in miss_spelled_to_correct_spelling SCI')
    ...
    for w in diva_merged_words_SCI.merged_words:
        wx=w.replace(' ', '')
        usage_cnt=1
    ...
    #
    added_to_unique_words_count=0
    for w in miss_spelled_to_correct_spelling_SCI.miss_spelled_to_correct_spelling:
        entry=miss_spelled_to_correct_spelling_SCI.miss_spelled_to_correct_spelling[w]
        if not entry:
            continue
    ...
    #
    print(f'{added_to_unique_words_count:>{Numeric_field_width}} added to the unique words based on those that occurred in miss_spelled_to_correct_spelling_SCI')
    
    add_words_file_name=f'{directory_prefix}extra_added_words_SCI.json'
    ...
    #
    # for some testing
    if options.testing and 'detection zone' in diva_merged_words_SCI.merged_words:
