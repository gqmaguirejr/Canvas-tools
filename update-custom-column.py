...
def update_column_name(course_id, column_id, column_name):
    global Verbose_Flag
    ...
    if r.status_code == requests.codes.ok:
        if Verbose_Flag:
            print("result of updating custom column:  {}".format(r.text))
...
def main():
    ...
    if (len(remainder) < 3):
        print("Inusffient arguments\n must provide course_id column_id custom_column_name\n")
    else:
        course_id=remainder[0]
        column_id=remainder[1]
        column_name=remainder[2]
        output=update_column_name(course_id, column_id, column_name)
        if (output):
            print(output)
