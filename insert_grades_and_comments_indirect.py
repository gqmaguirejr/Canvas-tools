...
#
# Note that the contents of the first column "Student" are not meaningful (you can just fill in NN).
#
...
def list_custom_columns(course_id):
    ...
    if r.status_code == requests.codes.ok:
        ...
        for p_response in page_response:  
            columns_found.append(p_response)
...
def lookup_column_number(column_name, list_of_existing_columns):
    for c in list_of_existing_columns:
        if c['title'] == column_name:
            return c['id']
    return -1



def get_custom_column_entries_all(course_id, column_number):
    global Verbose_Flag
    column_entries=[]
    ...
    if r.status_code == requests.codes.ok:
        ...
        for p_response in page_response:  
            column_entries.append(p_response)
...
def main():
    ...
    if (len(remainder) < 4):
        print("Insuffient arguments must provide course_id assignment_id file_name.csv indirect_column_name\n")
        return

    course_id=remainder[0]
    assignment_id=int(remainder[1])
    csv_file=remainder[2]
    indirect_column_name=remainder[3]
    ...
    column_number=lookup_column_number(indirect_column_name, list_of_existing_columns)
    if Verbose_Flag:
        print("column_number is {}".format(column_number))

    c_entries=get_custom_column_entries_all(course_id, column_number)
    if Verbose_Flag:
        print("c_entries are {}".format(c_entries))

    pseudo_id_to_user_id={}
    for e in c_entries:
        pseudo_id_to_user_id[e['content']]=e['user_id']

    if Verbose_Flag:
        print("pseudo_id_to_user_id is {}".format(pseudo_id_to_user_id))
    ...
    if len(grade_header) and len(comments_header) > 0:
        ...
        for index, row in gradebook_df.iterrows():
            name=row['Student']
            pseudo_user_id=row['ID']
            user_id=pseudo_id_to_user_id.get(pseudo_user_id, False) # translate from pseudo user ID to actual user_id
            if user_id:
                ...
                print("No user_id for pseudo user ID: {pseudo_user_id}".format(pseudo_user_id))
